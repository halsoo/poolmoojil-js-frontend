...
import { uploadImage, getBookByID, editBook } from '../../util/api';
...
export default class AdminStoreBookEdit extends Component {
    ...
    loadBook = async () => {
        ...
        if (res.status === 200) {
            let monthlyCurations = {};
            monthlyCurations =
                res.data.monthlyCurations.length === 0
                    ? { '0': null }
                    : res.data.monthlyCurations.map((m, index) => {
                          return (monthlyCurations[index] = m.date);
                      });
            this.setState({
                book: res.data,
                infos: {
                    ...
                    publishingCompany: res.data.publishingCompany,
